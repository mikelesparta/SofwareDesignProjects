...
public class EditorTest {
	...
	public void setUp() {
		...
		// se establece la predeterminada
		editor.setDefaultTool(selectionTool);
	}
	
	// Que no falle, en esta segunda versión, aun cuando no se le haya indicado
	// ninguna herramienta predeterminada
	//
	@Test
	public void testNoDefaultTool() {
		Editor editor = new Editor();
		assertNotNull(editor.getCurrentTool());
		try {
			editor.getCurrentTool().clickOn(100, 100);
			editor.getCurrentTool().moveTo(300, 200);
			editor.getCurrentTool().release();
		} catch (Exception e) {
			Assert.fail("No se debería haber lanzado una excepción aun sin herramienta predeterminada");
		}
	}
